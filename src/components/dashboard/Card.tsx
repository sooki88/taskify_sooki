...
function Card({ title, tags, dueDate, assignee, imageUrl, id }: CardProps) {
  ...
  return (
    <div
      className="flex flex-col w-full p-12 bg-white border cursor-pointer pc:flex-col tablet:flex-row pc:gap-y-12 tablet:gap-x-20 tablet:p-20 gap-y-10 rounded-6 border-gray-D9D9"
      ...
      <div className="flex flex-wrap w-full pc:flex-col tablet:flex-row tablet:gap-y-10 gap-y-6">
        <h2 className="w-full font-medium tablet:text-16 text-14 text-black-3332">{title}</h2>
        ...
        <div className="flex items-center justify-between grow">
          <div className="flex content-center gap-4 tablet:gap-6">
            ...
            <p className="font-medium tablet:text-12 text-10 text-gray-7874">{formatDate(dueDate)}</p>
          </div>
